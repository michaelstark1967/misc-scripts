...
This builds the image `alkira-aggregate:1.2.1`.
...
```bash
podman build -t alkira-aggregate:1.2.1 .
# forward TZ explicitly when running if desired
podman run --rm --env-file .env -e TZ="America/New_York" -v "$PWD":/app alkira-aggregate:1.2.1
